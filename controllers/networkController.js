...
const postNetwork = async (req, res, next) => {
    try {
        ...
        .status(201)
        .setHeader('Content-Type', 'application/json')
        .json({ device: network, message: `Device with name: ${network.deviceName} added. We will respond to the email associated with your account within 3 business days`})
    } catch (err) {
        next(err)
    }
